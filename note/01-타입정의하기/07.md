...
TS에서는 좀더 다양한 것들을 정의하는 데 쓰인다. TS에서 인터페이스로 정의할 수 있는 타입의 종류와 인터페이스로 타입을 정의하는 방법을 알아보자.

먼전 객체의 타입을 지정하는 방법을 알아보자.
...
아래는 선택 속성(optional property)을 지정하는 법이다.
...
하지만 똑같이 속성의 key가 문자열인 age는 error가 발생하는데 age는 index type이 아니라 명시적으로 정의된 `age: number`에 정의되기 때문이다.
...
자바스크립트에서는 속성 이름에 숫자와 문자열을 사용할 수 있지만 속성 이름에 숫자를 사용하면 내부적으로 문자열로 변환해서 사용한다.
...
반대로 interface에는 정의되지 않은 것을 class에서 추가하는 건 가능하다.
...
interface에 교차타입을 이용하면 하나로 합칠 수 있는데, 따라서 PP는 Person과 Product의 속성값을 모두 가지고 있다.

여기서 교집합인 게 속성값이 아니라 타입이 가질 수 있는 값의 집합의 교집합이기 때문에

PP가 name만 갖는 게 아니라 Person과 Product의 합처럼 기능한다.
